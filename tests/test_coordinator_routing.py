...
Reproduces the real bug: `everett route "what's the status of atlas"` picking a coordinator
session in ~ over the worker session in ~/Atlas, for both the local BM25 router and Jev.
"""
...
COORDINATOR = Session(
    ...
    title='Session sweep',
    card='Coordinator session: using Everett to list sessions and ask the Atlas session about its status.',
    first_user='use everett to check on my other sessions',
    last_user='everett_send: asking the atlas session for a status update',
)

WORKER = Session(
    'codex', 'work-1', '/Users/alice/Atlas', '/tmp/work-1.jsonl', '', NOW - 3600,
    title='Atlas ingest pipeline',
    card='Implementing the Atlas ingest pipeline; wiring up the scheduler next.',
    first_user='build the atlas ingest pipeline',
    last_user='wired up the scheduler for atlas ingest',
)

CALLER = Session(
    'claude', 'caller-1', '/Users/alice', '/tmp/caller-1.jsonl', '', NOW - 10,
    title='Routing question', card='Asking everett to route a request about atlas.',
)
...
class RelevantSessions(unittest.TestCase):
    def test_coordinator_excluded_when_project_not_named_by_it(self):
        pool = relevant_sessions("what's the status of atlas", [COORDINATOR, WORKER])
        self.assertNotIn(COORDINATOR, pool)
        self.assertIn(WORKER, pool)

    def test_caller_session_always_excluded(self):
        pool = relevant_sessions("what's the status of atlas", [WORKER, CALLER], caller_id='caller-1')
    ...
    def test_never_stranded_when_only_coordinators_exist(self):
        ...
        pool = relevant_sessions("what's the status of atlas", [COORDINATOR])
...
class LocalRouterPicksTheWorker(unittest.TestCase):
    def test_worker_session_wins_over_coordinator(self):
        r = local_route("what's the status of atlas", [COORDINATOR, WORKER], now=NOW)
        if r['decision'] == 'SESSION':
            ...
            self.assertIn('Atlas', r.get('suggested', '') or '')

    def test_caller_session_excluded_from_local_route(self):
        r = local_route("what's the status of atlas", [COORDINATOR, WORKER, CALLER],
...
class JevRouterPicksTheWorker(unittest.TestCase):
    ...
    def test_worker_only_candidate_when_coordinator_is_not_named(self):
        ...
        def fake_jev(text, crit, key):
            seen['crit'] = crit
            # Pick whichever option's description is about the actual Atlas work.
        ...
        r = route("what's the status of atlas", [COORDINATOR, WORKER], api_key='k', jev=fake_jev)
    ...
    def test_caller_session_excluded_from_jev_criteria(self):
        ...
        route("what's the status of atlas", [COORDINATOR, WORKER, CALLER], api_key='k',
